...
LOG_DIR="hpc_IC3REF_mab_alpha_1p0"
mkdir -p "$LOG_DIR"
...
process_file() {
	...
	COMMAND="bsub -Ip -n 1 -m "$CPU_HOSTS" /hpc/home/cwb.xzhoubu/IC3ref/IC3 -s -mab -alpha 1.0 < "$ABSOLUTE_FILE" 2>&1"
        # actually, IC3REF can be executed on any CPU HOSTS can does not require any docker to be pulled there
        bsub -Ip -n 1 "timeout 3600 /hpc/home/cwb.xzhoubu/IC3ref/IC3 -s -mab -alpha 1.0 < "$ABSOLUTE_FILE"" 2>&1
        echo "File: $FILE"
        echo "$COMMAND"
